...
class TextBox(WidgetBase):
    ...
    def __init__(
        ...
        *,
        ...
        highlightColour: tuple[int, int, int] = (166, 210, 255),
        fontSize: int = 20,
        font: pygame.font.Font | None = None,
        tabSpaces: int = 4,
        onSubmit: Callable[..., None] | None = None,
        onSubmitParams: tuple[Any, ...] = (),
        onTextChanged: Callable[..., None] | None = None,
        onTextChangedParams: tuple[Any, ...] = (),
        cursorWidth: int = 2,
    ) -> None:
        ...
        self.showCursor: bool = False
        self.cursorTime: float = 0.0
        self.cursorPosition: int = 0

        self.repeatTime: float = 0.0
        self.repeatKey: pygame.event.Event | None = None
        self.firstRepeat: bool = True
        self.keyDown: bool = False

        self.text: list[list[str]] = [[]]
        self.highlightedText: list[list[str]] = [[]]

        self.highlightStartLine: int = 0
        self.highlightEndLine: int = 0
        self.highlightStartInline: int = 0
        self.highlightEndInline: int = 0

        self.escape: bool = False

        self.borderThickness: int = borderThickness
        self.borderColour: tuple[int, int, int] = borderColour
        self.radius: int = radius

        self.colour: tuple[int, int, int] = colour
        self.cursorColour: tuple[int, int, int] = cursorColour

        self.isLabel: bool = isLabel
        self.placeholderText: str = placeholderText
        self.placeholderTextColour: tuple[int, int, int] = placeholderTextColour
        self.textColour: tuple[int, int, int] = textColour
        self.highlightColour: tuple[int, int, int] = highlightColour

        self.fontSize: int = fontSize
        self.font: pygame.font.Font = (
            font if font is not None else pygame.font.SysFont("calibri", self.fontSize)
        )

        self.tabSpaces: int = tabSpaces

        self.textOffsetTop: int = self.fontSize // 3
        self.textOffsetLeft: int = self.fontSize // 3
        self.textOffsetRight: int = self.fontSize // 2
        self.cursorOffsetTop: int = self._height // 6

        self.selectedLine: int = 0

        self.firstVisibleLine: int = 0
        self.maxVisibleLines: int = (
            self._height - self.textOffsetTop - self.borderThickness * 2
        ) // self.fontSize

        self.onSubmit: Callable[..., None] = (
            onSubmit if onSubmit is not None else lambda *_: None
        )
        self.onSubmitParams: tuple[Any, ...] = onSubmitParams

        self.onTextChanged: Callable[..., None] = (
            onTextChanged if onTextChanged is not None else lambda *_: None
        )
        self.onTextChangedParams: tuple[Any, ...] = onTextChangedParams

        self.cursorWidth: int = cursorWidth
        ...
        self._charWidthCache: dict[str, int] = {}
                ...
                if event.type == pygame.KEYDOWN:
                    ...
                    if event.key == pygame.K_BACKSPACE and not self.isLabel:
                        ...
                        in [
                            pygame.K_DELETE,
                            pygame.K_KP_PERIOD
                            if not event.mod & pygame.KMOD_NUM
                            else -1,
                        ]
    ...
    def updateRepeatKey(self) -> None:
        ...
        self.repeatKey = cast(Event, self.repeatKey)
        if self.firstRepeat:
    ...
    def addText(self, text: str) -> None:
        ...
        """
        text = str(text).replace("\t", " " * self.tabSpaces)
    ...
    def getLineWidth(self, line: int) -> list[float]:
        ...
        """
        x: list[float] = [self._x + self.textOffsetLeft]
...
if __name__ == "__main__":
    ...
    while run:
        ...
        AIgame.widgets.update(outerEvents)
